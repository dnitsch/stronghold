 ...
 */
BuildWorker.prototype.pipelineGericCliRunner = function (callback) {
    ...
    function doPipelineWork(input) {
        ...
        return when.promise((resolve, reject) => {
            const ansibleRunFn = function (input) {
                const __input = input;
                ServiceHelper.tcpSender.restAbstract(`${process.env.BASE_URL}orchestration/v2/generic/${__input.params.stage}/${__input.params.app_name}/${__input.params.commit_id}/${__input.params.provider || 'aws'}`, __input.body, {}, {}, __input.header, "PUT", (e, d) => {
                    if (e) {
                        return reject(e);
                    } else {
